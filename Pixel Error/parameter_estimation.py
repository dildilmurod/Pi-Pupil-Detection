...
"""
    Set parameter values according to best chosen parameter estimation
    The best chosen parameter should have the least error.
...
# dataset folder path
folder_path = r"dataset/LPW_example/LPW_subject4"
...
#canny_blur = [[i,j] for i in range(6,51,3) for j in range(7,27,2)]
canny_blur = [[i,27] for i in range(6,51,3)]

for CANNY_THRESHOLD, MEDIAN_BLUR_K_SIZE in canny_blur:
    print('/n/n')
    print("CANNY: ", CANNY_THRESHOLD, "  -  ", " BLUR: ", MEDIAN_BLUR_K_SIZE)
    read_file = folder_path + '/2.avi' 
    print(read_file)
    cap = cv.VideoCapture(read_file)
    print(cap)
    print('')
    column = ('x', 'y', 'extra')
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            output = draw_ellipse(output, contours_filtered)
    ...
    df_pupil_location.to_csv(folder_path + rf"/320PEst_Experiment_{int(file_name)}_canny_{CANNY_THRESHOLD}_blur_{MEDIAN_BLUR_K_SIZE}.csv", sep='\t', index=False)
    cap.release()
