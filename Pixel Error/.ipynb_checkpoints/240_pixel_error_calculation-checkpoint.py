...
for file in file_path:
        ...
        if ret:
            ...
            if not location:
                ...
                data_entry = pd.DataFrame.from_dict({
                    ...
                    'y': [location[0][1]],
                    'extra': [location[1:]]})
                #cv.imwrite(extra_pupil+".png", drawing)
                #extra_pupil = extra_pupil +'1'

                df_pupil_location = pd.concat([df_pupil_location, data_entry], ignore_index=True)
            
            #if not location:
                #cv.imwrite(final_out+".png", output)
            ...
            # Uncomment to see live pupil detection
            #cv.imshow("ROI", frame)
    ...
    df_pupil_location.to_csv(folder_path + rf"\240Experiment_{int(file_name)}_canny_{CANNY_THRESHOLD}_blur_{MEDIAN_BLUR_K_SIZE}.csv", sep='\t', index=False)
    cap.release()
